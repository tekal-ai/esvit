...
class DepthWiseConv2d(nn.Module):
    def __init__(self, dim_in, dim_out, kernel_size, padding, stride, bias=True):
        ...
        self.pw = nn.Conv2d(dim_in, dim_out, kernel_size=1, bias=bias)
...
class Attention(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        q, k, v = map(
            lambda t: rearrange(
                t,
                "b (h d) (s_x w_x) (s_y w_y) -> (b s_x s_y) h (w_x w_y) d",
                h=h,
                s_x=s_x,
                s_y=s_y,
                w_x=w,
        ...
        if self.rel_pos_embed:
            rel_pos_bias = self.rel_pos_bias_table[self.rel_pos_idx.view(-1)].view(
                self.window_size * self.window_size,
                self.window_size * self.window_size,
                -1,
            )  # Wh*Ww,Wh*Ww,nH
            rel_pos_bias = rel_pos_bias.permute(
                2, 0, 1
        ...
        if mask is not None:
            nW = mask.shape[0]
            B_, H, N, M = dots.shape
            dots = dots.view(B_ // nW, nW, self.heads, N, M) + mask.unsqueeze(
                1
            ).unsqueeze(0)
        ...
        out = rearrange(
            out,
            "(b s_x s_y) h (w_x w_y) d -> b (h d) (s_x w_x) (s_y w_y)",
            h=h,
            s_x=s_x,
            s_y=s_y,
            w_x=w,
...
class Transformer(nn.Module):
    ...
    def build_attn_mask(self, x):
        ...
        mask_windows = rearrange(
            img_mask,
            "i (s_x w_x) (s_y w_y) j -> (i s_x s_y) w_x w_y j",
            s_x=s_x,
            s_y=s_y,
            w_y=self.window_size,
            w_x=self.window_size,
        )
    ...
    def __init__(self, channels_stem, deep=False):
        ...
        if deep:
            self.stem = nn.Sequential(
                nn.Conv2d(
                    3, channels_stem, kernel_size=3, stride=2, padding=1, bias=False
                ),
...
class CvT(nn.Module):
    def __init__(
        ...
        *,
        ...
        for i in range(self.num_stages):
            ...
            stage = nn.Sequential(
                ...
                Transformer(
                    ...
                    drop_path_rate=dpr[depth_accum : depth_accum + kwargs["depth"]],
                    act_layer=act_layer,
                    norm_layer=norm_layer,
        ...
        self.head = (
            nn.Linear(in_chans, num_classes) if num_classes > 0 else nn.Identity()
        )
